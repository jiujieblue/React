require('../../less/daifa-deal-detail.less');
...
var DaifaDealDetailUtil = require('../utils/DaifaDealDetailUtil.js');
var DaifaDealDetail = React.createClass({
	...
	componentDidMount:function(){
		DaifaDealDetailUtil.getData(function(res){
			this.setState({
				res:res,
				logs:res.logs,
	...
	_submit:function(e){
		...
		if(data.timeStart || data.timeEnd || data.paymentSerialNum || data.phone || data.money || Boolean(Number(data.type))){
			DaifaDealDetailUtil.search(data,function(res){
				me.setState({
					logs:res.logs
				})
			})
		...
		return (
			<div className="row">
				...
				<div className="col-md-10">
					<div className="daifa-deal-detail">
						<div className="daifa-deal-detail-location">
							当前位置：我的钱包 > 交易明细
						</div>
						<form className="daifa-deal-detail-menu" onSubmit={this._submit}>
						...
						<div className="daifa-deal-detail-name">
							<table>
								<thead>
									<tr>
										<td>序号</td>
...
module.exports = DaifaDealDetail;