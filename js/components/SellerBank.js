require('../../less/seller-bank.less');
var React = require('react');
var SellerSide = require('./SellerSide.js');
var Modal = require('react-modal');
var SellerBankUtil = require('../utils/SellerBankUtil.js');
var SellerBank = React.createClass({
	...
	componentDidMount:function(){
		var me = this;
		SellerBankUtil.getData(function(res){
			me.setState({
				data:res
			})
		})
    ...
    _delCard:function(){
		...
		SellerBankUtil.delCard(formData, function(res) {
	...
	_addCard:function(t,e){
		...
		if(t==-1){
			...
			SellerBankUtil.addCard({},function(res){
				console.log(res)
			})
		}
		else{
			...
			SellerBankUtil.addCard(formData,function(res){
				me.setState({
					addCardData:res,
	...
	render: function () {
		...
		var itemsData = me.state.data.map(function(item,index){
			return(
				<div className="seller-bank-card-detail" key={index}>
			...
			<div className="row">
				...
				<div className="col-md-10">
					<div className="seller-bank">
						<div className="seller-bank-location">
							当前位置：我的钱包 > 银行卡管理
						</div>
						<div style={{display:this.state.type == 1 ? 'block' : 'none'}}>
							<div className="seller-bank-title">
								我的银行卡
							</div>
							<div className="seller-bank-card">
								{itemsData}
								<div className="seller-bank-card-add" onClick={this._addCard.bind(this,-1)}>
									<h3>+</h3>
									<p>添加银行卡</p>
								</div>
							</div>
						...
						<div style={{display:this.state.type == 2 ? 'block' : 'none'}}>
							<form className="seller-bank-addcard" onSubmit={this._submit}>
						...
						<div style={{display:this.state.type == 3 ? 'block' : 'none'}}>
							<div className="seller-bank-addcardsucc">
