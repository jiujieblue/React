require('../../less/seller-withdrawal.less');
var React = require('react');
var SellerSide=require('./SellerSide.js');
var ErrorMsg = require('./ErrorMsg.js');
var SellerWithdrawalUtil=require('../utils/SellerWithdrawalUtil.js');
var SellerWithdrawal = React.createClass({
	...
	componentDidMount:function(){
		var me = this;
		SellerWithdrawalUtil.getWithdrawalData(function(res){
			if(!res.isActive){
				me.setState({
					isDisabled: 'disabled',
	...
	_submit:function(e){
		...
		if(!this.refs.money.value||!regM.test(this.refs.money.value)){
			...
			SellerWithdrawalUtil.sellerLogin(data,function(res){
				console.log(res);
		...
		return (
			<div className="row">
				...
				<div className="col-md-10">
					<div className="seller-withdrawal">
								...
								<ul>
									<li>
										...
										<div>
											...
											<i ref='rotate' style={{display:this.state.cardList.length==0?'none':'block'}}></i>
											<ul ref='cardList'>
												{this.state.cardList.map(function(item,index){
													return (
									...
									<li><span></span><a href='#'>添加银行卡</a></li>
								</ul>
								<ul>
									<li>
										<span>可提现余额：</span>{this.state.cardData.balance}
									</li>
									<li ref='errorMoney'>
										<span>提 现 金 额：</span>
									...
									<li ref='errorPwd'>
										<span>支 付 密 码：</span>
										<input type='password' placeholder='请输入支付密码' disabled={this.state.isDisabled} name='pwd' ref='pwd' onBlur={this.handleManeyBlur.bind(this,0)}/>
